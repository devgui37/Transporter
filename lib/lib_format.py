...
from rich.table import Table
import lib.lib_transport as lt
import pandas as pd
import numpy as np


def client_res(
    data: pd.DataFrame, nb_camion: int = None, camion: float = None
) -> Table():
...
def entrepot_res(data: pd.DataFrame) -> Table():
...
def total_cout(data: pd.DataFrame) -> float:
    """
